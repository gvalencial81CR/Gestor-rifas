...
# --- VISTA PRINCIPAL (CLIENTE Y NAVEGACIÓN) ---
...
# Pestana 1: Comprar Números
with tab1:
    mapa_ocupados = obtener_mapa_numeros_ocupados()
    disponibles = total_numeros_config - len(mapa_ocupados)

    if disponibles <= 10:
        st.warning(t["disp_quedan"].format(disponibles))
    else:
        st.info(t["disp_total"].format(disponibles, total_numeros_config))
    ...
    # Grilla de selección
    cols_por_fila = 10
    numeros_lista = [f"{i:02d}" for i in range(total_numeros_config)]

    # Manejo del estado global de selección
    for i in range(0, len(numeros_lista), cols_por_fila):
        cols = st.columns(cols_por_fila)
        for j, col in enumerate(cols):
            idx = i + j
            if idx < len(numeros_lista):
                num_str = numeros_lista[idx]
                ocupado = num_str in mapa_ocupados
                estado = mapa_ocupados.get(num_str, "")
                ...
                with col:
                    if ocupado:
                        ...
                        label = (
                            f"🟢\n{num_str}" if is_selected else f"⚪\n{num_str}"
                        )
                        if st.button(label, key=f"btn_{num_str}"):
                            if is_selected:
                                ...
                                if (
                                    len(st.session_state.seleccionados_global)
                                    < max_numeros_permitidos
                                ):
                                    ...
                                    st.toast(
                                        t["limite_alcanzado"].format(
                                            max_numeros_permitidos
                                        )
                                    )
                            st.rerun()

    seleccionados = sorted(st.session_state.seleccionados_global)

    if seleccionados:
        total_pago = len(seleccionados) * precio_numero
        st.write("---")
        st.markdown(
            f"### {t['nums_elegidos']}: **{', '.join(seleccionados)}**"
        )
        st.markdown(f"### {t['total_pagar']} **₡{total_pago:,.0f} CRC**")

        st.write("---")
        st.subheader(t["datos_reserva"])
        nombre_cliente = st.text_input(t["tu_nombre"], key="nombre_cli")
        telefono_cliente = st.text_input(t["tu_telefono"], key="tel_cli")

        if st.button(t["btn_confirmar"]):
            tel_limpio = re.sub(r"\D", "", telefono_cliente)
            if not nombre_cliente.strip():
                st.error(t["err_nombre"])
            elif len(tel_limpio) != 8:
                st.error(t["err_telefono"])
            else:
                exitos, fallos = guardar_reserva(
                    seleccionados, nombre_cliente, tel_limpio
                )
                if exitos:
                    st.session_state.reserva_confirmada = True
                    st.session_state.ultimo_comprador = nombre_cliente
                    st.session_state.ultimo_telefono = tel_limpio
                    st.session_state.ultimos_numeros = exitos
                    st.session_state.seleccionados_global = []
                    st.rerun()
                if fallos:
                    st.error(f"{t['nums_ocupados']} {', '.join(fallos)}")

    if st.session_state.reserva_confirmada:
        st.balloons()
        nums_str = ", ".join(st.session_state.ultimos_numeros)
        st.success(t["reserva_exito_varios"].format(nums_str))

        total_pago = len(st.session_state.ultimos_numeros) * precio_numero

        # Opciones de pago
        st.markdown(f"### {t['datos_sinpe']}")
        st.write(f"📲 **Número SINPE:** {num_limpio}")
        st.write(f"👤 **{t['titular']}** {nombre_sinpe}")
        st.write(f"💰 **Total:** ₡{total_pago:,.0f} CRC")

        msg_wa = (
            f"Hola, reservé el/los número(s) {nums_str} para la"
            f" {titulo_rifa}. Mi nombre es"
            f" {st.session_state.ultimo_comprador}. Adjunto comprobante de"
            " pago."
        )
        url_wa = (
        ...
        st.markdown(
            ...
            <a href="{url_wa}" target="_blank">
                <button style="background-color: #25D366; color: white; border: none; padding: 12px; font-weight: bold; border-radius: 6px; width: 100%; cursor: pointer;">
                    {t['btn_confirmar_wa']}
                </button>
            </a>
            """,
...
# Pestana 2: Premio Único
with tab2:
    st.subheader(t["premio_unico"])
    premios = obtener_premios()
    if premios:
        for p in premios:
            _, lugar, nombre, desc, img_b64 = p
...
# Pestana 3: Reglamento
with tab3:
    st.subheader("📜 Reglamento de Participación")
